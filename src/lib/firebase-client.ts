...
export async function sendClientPasswordReset(email: string) {
  ...
  if (!response.ok || result.ok === false) {
    ...
    if (detail.includes("Firebase Admin credentials")) {
      throw new Error("Nearwork needs to finish connecting the client portal email service before this reset can be sent. Please contact support@nearwork.co.");
    }
    throw new Error(detail || "Could not send password reset.");
  }
}
