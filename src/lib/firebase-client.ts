...
export async function createClientAccount(email: string, password: string) {
  ...
  if (!inviteSnap.empty) {
    ...
    await setDoc(doc(db, "users", credential.user.uid), {
      ...
      name: invite.name || credential.user.displayName || normalizedEmail,
      firstName: invite.firstName || "",
      lastName: invite.lastName || "",
      role: invite.role || "client",
      portalRole: invite.portalRole || "viewer_client",
      orgId: invite.orgId,
      organizationId: invite.organizationId || invite.orgId,
      orgName: invite.orgName,
      businessRole: invite.businessRole || invite.title || "",
      title: invite.title || invite.businessRole || "",
    ...
    await setDoc(doc(db, "orgInvites", inviteDoc.id), {
      status: "active",
      invitePending: false,
