...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    async function verify() {
      if (!oobCode) {
        setError("This password setup link is missing a reset code. Ask Nearwork to resend your invitation.");
        return;
      }
      try {
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#f6f8fa] px-5 py-10 text-[#24292f]">
      <form onSubmit={submit} className="w-full max-w-md rounded-lg border border-[#d8dee4] bg-white p-6 shadow-sm">
        ...
        <h1 className="mt-2 text-3xl font-black">Create your password</h1>
        ...
        <label className="mt-5 block text-sm font-bold">
          Email
          <input value={email} readOnly type="email" className="mt-2 h-11 w-full rounded-md border border-[#d8dee4] bg-[#f6f8fa] px-3 text-[#57606a] outline-none" />
        </label>
        ...
        <button disabled={!ready || busy} className="mt-5 h-11 w-full rounded-md bg-[#12866E] text-sm font-black text-white disabled:opacity-60">
          {busy ? "Saving..." : ready ? "Save password" : "Checking link..."}
        </button>
      </form>
    </main>
  );
