...
// ── New client-portal design — Billing screen (spend + invoices + per-hire) ───
// Ported from portal-v3-billing.jsx (BillingScreen). The prototype read the
// window.NW_BILLING / NW_INV_STATUS / NW_SPP_BILLING / NW_SPP_CLIENTS globals;
// here the screen takes a typed `data` object + `client` as PROPS so real
// Firebase / partnerBilling data drops in later. Inline styles kept verbatim for
// fidelity. Not a SaaS: no credits/usage — just what the client spends per month
// with a per-service + per-hire breakdown and invoice history.
...
export type BillingInvoiceStatus = "paid" | "open" | "upcoming";

// One point on the 6-month spend trend chart.
export type BillingTrendPoint = { m: string; v: number };

// A row in the "US comparison" band — same service layer priced in the US.
export type BillingUsComparison = { equivalent: string; savings: string; savingsPct: string };

// A slice of "Spend by service" (managed team / EOR / direct recruitment).
...
// One hire's recurring monthly cost (managed fee + EOR tier) — per-hire breakdown.
export type BillingPerHire = {
  id: string | number;
  name: string;
  initials: string;
  avatarBg: string;
...
// One monthly invoice — invoice history.
...
// SPP (Strategic Partner Program) — what the partner pays Nearwork.
export type BillingSpp = {
  subscription: string;
  tier: string;
...
// One end-client the SPP partner resells Nearwork to (partner-only view).
export type BillingSppClient = {
  id: string | number;
  name: string;
  initials: string;
  logoBg: string;
...
export type BillingData = {
  ...
  due: string;
  status: BillingInvoiceStatus | string;
  us: BillingUsComparison;
  services: BillingService[];
  trend: BillingTrendPoint[];
  perHire: BillingPerHire[];
  perHireVisible?: number;
  invoices: BillingInvoice[];
  ytd: string;
  // Optional SPP partner billing — only rendered for reseller partners.
  spp?: BillingSpp | null;
  sppClients?: BillingSppClient[];
};

type NavHandler = (id: string, arg?: string | number) => void;

// ── Invoice status styling (verbatim from NW_INV_STATUS) ──────────────────────
const INV_STATUS: Record<string, { label: string; color: string; bg: string; dot: string }> = {
...
function SpendChart({ trend, dense }: { trend: BillingTrendPoint[]; dense?: boolean }) {
...
// ── Billing screen ────────────────────────────────────────────────────────────
export function BillingScreen({ client, data, density = "regular", onNav }: {
  client: PortalClient;
  data: BillingData;
  ...
  const b = data;
  const [view, setView] = useState<"total" | "normal" | "spp">("total");

  // No-billing empty state.
  const populated = !!b && !!b.month;
  const st = (b && INV_STATUS[b.status]) || INV_STATUS.open;
  const services = b?.services ?? [];
  const trend = b?.trend ?? [];
  const perHire = b?.perHire ?? [];
  const perHireVisible = b?.perHireVisible ?? perHire.length;
  const invoices = b?.invoices ?? [];
  const spp = b?.spp ?? null;
  const sppClients = b?.sppClients ?? [];

  return (
    <div style={{ display: "flex", width: "100%", height: "100%", background: NW.offWhite, color: NW.black, fontFamily: "Poppins, sans-serif" }}>
      <PortalSidebar active="billing" density={density} onNav={onNav} client={client} />
          ...
          <div style={{ maxWidth: 1120, margin: "0 auto" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: dense ? 22 : 30, gap: 24 }}>
              <div>
                ...
                <h1 style={{ fontSize: dense ? 34 : 44, fontWeight: 700, color: NW.black, letterSpacing: "-0.04em", lineHeight: 1.02, margin: 0, fontFamily: "Poppins, sans-serif" }}>Spend &amp; invoices</h1>
            ...
            {!populated ? (
              <EmptyBlock icon="wallet" title="No billing yet"
                desc="Once you start hiring or employing through Nearwork, your monthly spend, per-service breakdown and invoices will appear here." />
            ) : (
              <>
                {/* View filter */}
                <div style={{ display: "flex", gap: 3, padding: 4, background: NW.white, border: `1px solid ${NW.gray100}`, borderRadius: 10, width: "fit-content", marginBottom: dense ? 16 : 20, boxShadow: "0 1px 2px rgba(0,0,0,0.03)" }}>
                  {[{ id: "total", label: "Total" }, { id: "normal", label: "Normal" }, { id: "spp", label: "SPP only" }].map((o) => {
                    const on = view === o.id;
                    return <button key={o.id} onClick={() => setView(o.id as "total" | "normal" | "spp")} style={{ border: "none", padding: "7px 16px", borderRadius: 7, fontFamily: "inherit", fontSize: 12.5, fontWeight: 600, cursor: "pointer", background: on ? NW.black : "transparent", color: on ? NW.white : NW.gray600, transition: "background 150ms, color 150ms" }}>{o.label}</button>;
                  })}
                </div>
                  ...
                  <>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: dense ? "1fr" : "1fr 1.4fr", gap: dense ? 16 : 22, marginBottom: dense ? 16 : 22 }}>
                      ...
                      <div style={{ background: NW.white, border: `1px solid ${NW.gray100}`, borderRadius: 20, padding: dense ? 24 : 30 }}>
                        ...
                        {trend.length > 0
                          ? <SpendChart trend={trend} dense={dense} />
                          : <div style={{ fontSize: 13, color: NW.gray400, padding: "20px 0" }}>No spend history yet.</div>}
                      </div>
                    </div>
                    ...
                    {services.length > 0 && (
                      <section style={{ background: NW.white, border: `1px solid ${NW.gray100}`, borderRadius: 20, padding: dense ? 24 : 30, marginBottom: dense ? 16 : 22 }}>
                        ...
                        <div style={{ display: "flex", gap: 3, height: 14, marginBottom: 20 }}>
                          {services.map((s) => <div key={s.id} title={`${s.label}: ${s.amount}`} style={{ flex: s.pct, background: s.color, borderRadius: 5 }} />)}
                        </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: dense ? "1fr" : "1.3fr 1fr", gap: dense ? 16 : 22 }}>
                        ...
                        {perHire.length > 0 ? (
                          <>
                            <div style={{ display: "flex", flexDirection: "column" }}>
                              {perHire.slice(0, perHireVisible).map((h, i) => (
                            ...
                            {perHire.length > perHireVisible && (
                              <button onClick={() => onNav && onNav("team")} style={{ width: "100%", marginTop: 10, border: `1px solid ${NW.gray200}`, background: NW.white, borderRadius: 10, padding: "10px", fontFamily: "inherit", fontSize: 12.5, fontWeight: 600, color: NW.gray700, cursor: "pointer", display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 7 }}>
                                View all {perHire.length} on Team <Icon name="arrow-right" size={14} color={NW.gray600} />
                              </button>
                            )}
                          </>
                        ) : (
                          <div style={{ fontSize: 13, color: NW.gray400, padding: "16px 4px" }}>No hires billed this month yet.</div>
                        )}
                      </section>
                      ...
                      <section style={{ background: NW.white, border: `1px solid ${NW.gray100}`, borderRadius: 20, padding: dense ? "24px 18px" : "28px 22px" }}>
                        ...
                        {invoices.length > 0 ? (
                          ...
                          <div style={{ fontSize: 13, color: NW.gray400, padding: "16px 4px" }}>No invoices yet.</div>
                        )}
                      </section>
                    </div>
                  </>
                ...
                {view !== "normal" && spp && (
                  <section style={{ background: NW.white, border: `1px solid ${NW.gray100}`, borderRadius: 20, padding: dense ? 24 : 30, marginTop: dense ? 16 : 22 }}>
                    ...
                    <p style={{ fontSize: 12.5, color: NW.gray500, margin: "0 0 18px", maxWidth: 640, lineHeight: 1.5 }}>{spp.discountNote}</p>
                    ...
                    <div style={{ display: "flex", gap: dense ? 14 : 18, marginBottom: dense ? 18 : 22, flexWrap: "wrap" }}>
                      <div style={{ flex: 1, minWidth: 200, background: "linear-gradient(160deg, #FFF, #AF7AC50d)", border: "1px solid #AF7AC533", borderRadius: 14, padding: "16px 18px" }}>
                        ...
                        <div style={{ fontFamily: "Poppins", fontSize: 28, fontWeight: 700, color: NW.black, letterSpacing: "-0.03em" }}>{spp.subscription}<span style={{ fontSize: 13, color: NW.gray500, fontWeight: 500 }}> /mo</span></div>
                        <div style={{ fontSize: 11.5, color: NW.gray500, marginTop: 3 }}>{spp.tier}</div>
                      </div>
                      <div style={{ flex: 1, minWidth: 200, background: NW.gray50, border: `1px solid ${NW.gray100}`, borderRadius: 14, padding: "16px 18px" }}>
                        ...
                        <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
                          <span style={{ fontFamily: "Poppins", fontSize: 28, fontWeight: 700, color: NW.teal600, letterSpacing: "-0.03em" }}>{spp.placementSpp}</span>
                          <span style={{ fontSize: 14, color: NW.gray400, textDecoration: "line-through" }}>{spp.placementList}</span>
                        </div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 16, paddingTop: 16, borderTop: `1px solid ${NW.gray100}` }}>
                      <span style={{ fontSize: 13, fontWeight: 600, color: NW.gray600 }}>Total to Nearwork · subscription + {sppClients.length} clients</span>
                      <span style={{ fontFamily: "Poppins", fontSize: 24, fontWeight: 700, color: NW.black, letterSpacing: "-0.03em" }}>{spp.total}<span style={{ fontSize: 13, color: NW.gray500, fontWeight: 500 }}> /mo</span></span>
                    </div>
                  </section>
                )}
              </>
