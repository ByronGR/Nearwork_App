...
// ── New client-portal design — Team screen (People + Teams) & Team detail ─────
// Ported from portal-v3-team.jsx (TeamScreen + TeamDetailScreen). The prototype
// read window.NW_TEAM_PEOPLE / window.NW_TEAM_TEAMS / window.NW_CLIENT globals;
// here every screen takes a typed `data` object + `client` as PROPS so real
// Firebase data drops in later. Inline styles kept verbatim for fidelity.
...
export type TeamPersonStatus = "active" | "onleave" | "offboarded";
export type TeamSeniority = "Senior" | "Mid" | "Junior";
...
export type TeamPerson = {
  ...
  location: string;
  teamId?: string | null;
  status: TeamPersonStatus | string;
  since: string;
  tenure: string;
  managed?: boolean;
  statusNote?: string;
  unread?: number; // count of new updates for this hire (drives red badge)
};

export type TeamLead = { name: string; initials: string; avatarBg: string };
export type TeamAccountManager = { name: string; initials: string; avatarBg: string; role?: string };
...
export type TeamGroup = {
  ...
  lead: TeamLead;
  pod?: string | null;
  health: string; // "on-track" | "attention" | …
  accent: string;
  accountManager: TeamAccountManager;
  hoursPartner: string;
...
export type TeamData = {
  people: TeamPerson[];
  teams: TeamGroup[];
  partnerTzLabel?: string; // client's short timezone label (e.g. "CST"); defaults to "Partner"
};

export type TeamFocus = { view?: "people" | "teams" };

type NavHandler = (id: string, arg?: string | number) => void;

// ── Status + seniority styling maps ───────────────────────────────────────────
...
function PersonRow({ p, dense, teamName, onOpen }: {
  p: TeamPerson;
  dense?: boolean;
  last?: boolean;
  teamName?: string;
...
// ── Team screen ─────────────────────────────────────────────────────────────
export function TeamScreen({ client, data, density = "regular", onNav, focus }: {
  client: PortalClient;
  data: TeamData;
  ...
  const [view, setView] = useState<"people" | "teams">(focus && focus.view ? focus.view : "people");
  const people = data.people;
  const teams = data.teams;
  const populated = people.length > 0;
    ...
    <div style={{ display: "flex", width: "100%", height: "100%", background: NW.offWhite, color: NW.black, fontFamily: "Poppins, sans-serif" }}>
      ...
      <main style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, overflow: "auto", padding: `${dense ? 28 : 40}px ${pad}px ${pad}px` }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: dense ? 22 : 28 }}>
            <div style={{ display: "flex", gap: 3, padding: 4, background: NW.white, border: `1px solid ${NW.gray100}`, borderRadius: 12, boxShadow: "0 1px 2px rgba(0,0,0,0.03)" }}>
              {[
                ...
                const on = view === v.id;
                return (
                  <button key={v.id} onClick={() => setView(v.id as "people" | "teams")} style={{
          ...
          {/* Body */}
          {!populated ? (
            <EmptyBlock icon="handshake" title="No one on your team yet"
...
function TeamMeta({ icon, label, children }: { icon: string; label: string; children?: React.ReactNode }) {
  return (
...
export function TeamDetailScreen({ client, data, teamId, density = "regular", onNav }: {
  client: PortalClient;
  data: TeamData;
  teamId?: string;
  density?: "regular" | "compact";
  ...
  const members = (data.people || []).filter((p) => team && p.teamId === team.id);
  const activeCount = members.filter((m) => m.status === "active").length;
  const healthOk = team ? team.health === "on-track" : false;
  const [tz, setTz] = useState<"partner" | "colombia">("partner");
  const partnerLabel = data.partnerTzLabel || "Partner";
  if (!team) return null;
              ...
              <TeamMeta icon="clock" label="Working hours">
                ...
                <div style={{ display: "flex", gap: 2, padding: 3, background: NW.gray50, border: `1px solid ${NW.gray100}`, borderRadius: 8 }}>
                  {[
                    ...
                    const on = tz === o.id;
                    return (
                      <button key={o.id} onClick={() => setTz(o.id as "partner" | "colombia")} style={{ flex: 1, border: "none", padding: "5px 6px", borderRadius: 6, fontFamily: "inherit", fontSize: 11, fontWeight: 600, cursor: "pointer", background: on ? NW.white : "transparent", color: on ? NW.black : NW.gray500, boxShadow: on ? "0 1px 2px rgba(0,0,0,0.06)" : "none" }}>{o.label}</button>
                    );
                  })}
                </div>
              </TeamMeta>
