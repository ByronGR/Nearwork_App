...
const tabs = [
  ...
  { id: "openings", label: "Openings", icon: BriefcaseBusiness, section: "Recruiting" },
  { id: "pipeline", label: "Pipeline", icon: ClipboardList, section: "Recruiting" },
...
type TabId = (typeof tabs)[number]["id"] | "candidate";
...
const pipelineStages = [
  ...
  { key: "client-review", label: "Company Review" },
    ...
    <main className="grid min-h-screen bg-[#f6f8fa] px-5 py-8 text-[#24292f] lg:grid-cols-[1fr_440px]">
      ...
      <section className="flex items-center justify-center px-0 py-8 lg:px-10">
        <form onSubmit={submit} className="w-full rounded-lg border border-[#d8dee4] bg-white p-6 shadow-sm">
          ...
          <h1 className="mt-2 text-3xl font-black">Company login</h1>
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge tone={stageTone[stage] || "border-stone-200 bg-stone-50 text-stone-700"}>{stage === "client-review" ? "Company review" : candidate.stage || "Review"}</Badge>
          </div>
...
export function ClientPortal() {
  ...
  const reviewCount = pipelineCandidates.filter(({ candidate }) => ["presented", "client-review"].includes(String(candidate.stage || "").toLowerCase())).length;
  const estimatedMonthlySavings = Math.max(0, visibleHires.length || hiredCount) * 1800;
  const accountManager = {
    name: org?.accountManagerName || demoAccountManager.name,
    email: org?.accountManagerEmail || demoAccountManager.email,
    phone: org?.accountManagerPhone || demoAccountManager.phone,
  };
  ...
  const globalResults = useMemo(() => {
    ...
    const openingRows = openings
      ...
      .slice(0, 4)
      .map((opening) => ({ key: `opening-${opening.code}`, type: "Opening", title: opening.title, sub: opening.code, action: () => setActive("openings") }));
    ...
    const hireRows = visibleHires
      ...
      .slice(0, 3)
      .map((hire) => ({ key: `hire-${hire.id}`, type: "Hire", title: hire.candidateName || hire.name || "Hire", sub: `${hire.role || "Role"} · ${hire.serviceType || hire.engagementType || "Service"}`, action: () => setActive("hires") }));
    return [...candidateRows, ...openingRows, ...hireRows];
  }, [search, openings, pipelineCandidates, visibleHires]);
      ...
      <main className={cx("min-h-screen transition-[margin] duration-200", sidebarOpen ? "ml-72" : "ml-20")}>
        <header className="sticky top-0 z-10 border-b border-[#d8dee4] bg-white/95 px-4 py-3 backdrop-blur lg:px-8">
          <div className="mx-auto flex max-w-7xl items-center gap-4">
            <div className="relative flex min-w-[280px] flex-1 items-center gap-3">
        ...
        <div className="mx-auto max-w-7xl space-y-6 px-5 py-7 lg:px-10">
          ...
          {active === "pipeline" ? (
            selectedPipeline ? (
              ...
              <PipelineList pipelines={pipelines} rows={pipelineCandidates} onOpen={(code) => { setSelectedPipelineCode(code); setSelectedCode(""); }} />
            )
          ) : null}
...
function PipelineBoard({
  ...
  return (
    <div className="overflow-x-auto pb-2">
      <div className="grid min-w-[1840px] grid-cols-8 gap-4">
...
function PipelineList({
  ...
  if (!pipelines.length) return <Panel title="Pipelines" eyebrow="Recruiting"><Empty title="No pipelines yet" text="When Nearwork creates pipelines for your company, they will appear here." /></Panel>;
  return (
    <Panel title="Pipelines" eyebrow="Active and closed">
        ...
        {pipelines.map((pipeline) => {
          ...
          return (
            <button key={pipeline.code} onClick={() => onOpen(pipeline.code)} className="rounded-xl border border-[#d8dee4] bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-[#12866E] hover:shadow-md">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.16em] text-[#12866E]">{active ? "Active pipeline" : "Closed pipeline"}</p>
                  <h3 className="mt-2 text-xl font-black">{pipeline.openingTitle || pipeline.code}</h3>
                  <p className="mt-1 text-sm text-[#57606a]">{pipeline.code} · Recruiter: {pipeline.recruiter || "Nearwork team"}</p>
                </div>
              ...
              <div className="mt-5 grid grid-cols-3 gap-3">
                ...
                <MiniStat label="Review" value={candidates.filter(({ candidate }) => ["presented", "client-review"].includes(String(candidate.stage || "").toLowerCase())).length} />
                <MiniStat label="Hired" value={candidates.filter(({ candidate }) => String(candidate.stage || "").toLowerCase() === "hired").length} />
              </div>
            </button>
          );
        })}
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <Panel title="Assessment detail" eyebrow="Score explanation">
          <p className="font-black">{breakdown.signal}</p>
          <p className="mt-2 text-sm leading-6 text-[#57606a]">{breakdown.focus}</p>
          <div className="mt-4 rounded-lg border border-[#d8dee4] bg-[#f6f8fa] p-4 text-sm text-[#57606a]">
            The current breakdown is based on the stored assessment score. When kickoff notes and question-level scoring are connected, this section can explain the exact skill gaps for {pipeline.openingTitle || "this role"}.
          </div>
        </Panel>
        <Panel title="DISC evaluation" eyebrow={disc.title}>
          <div className="space-y-3 text-sm leading-6 text-[#57606a]">
            <p><span className="font-black text-[#24292f]">Benefits:</span> {disc.benefit}</p>
            <p><span className="font-black text-[#24292f]">Watch-outs:</span> {disc.watchout}</p>
            <p><span className="font-black text-[#24292f]">Role fit:</span> {disc.fit}</p>
          </div>
        </Panel>
      </section>
    ...
    <div className="fixed inset-0 z-40 grid place-items-center bg-black/20 p-4">
      <section className="w-full max-w-lg rounded-xl border border-[#d8dee4] bg-white p-6 shadow-2xl">
        ...
        <div className="mt-5 grid gap-4">
          <label className="text-sm font-black">Name<input className="mt-2 h-11 w-full rounded-md border border-[#d8dee4] px-3 font-normal outline-none focus:border-[#12866E]" placeholder="Jane Smith" /></label>
        ...
        <p className="mt-3 text-xs leading-5 text-[#6e7781]">This UI captures role context now. The backend invite write can be connected to Firebase once the org-user invite collection is finalized.</p>
      </section>
    </div>
  );
}
