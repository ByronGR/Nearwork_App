      ...
      >
        <div className="border-b border-[#d8dee4] p-4 lg:p-6">
          <div className="flex items-center justify-between gap-3">
            ...
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="grid size-9 place-items-center rounded-md border border-[#d8dee4] bg-white text-[#57606a]">
        ...
        <div className="border-t border-[#d8dee4] p-3 lg:p-4">
          ...
          <div className="rounded-lg border border-[#d8dee4] bg-[#f6f8fa] p-3">
            <p className="text-xs font-black uppercase tracking-[0.14em] text-[#12866E]">Need support?</p>
            <p className="mt-2 font-bold">{accountManager.name}</p>
            <p className="mt-1 text-xs text-[#6e7781]">{accountManager.email}</p>
            <p className="mt-1 text-xs text-[#6e7781]">{accountManager.phone}</p>
        ...
        <header className="sticky top-0 z-10 border-b border-[#d8dee4] bg-white/95 px-4 py-3 backdrop-blur lg:px-8">
          <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4">
            <div className="flex min-w-[280px] flex-1 items-center gap-3">
              <button onClick={() => setSidebarOpen(!sidebarOpen)} className="grid size-10 place-items-center rounded-md border border-[#d8dee4] bg-white text-[#57606a]">
                <Menu className="size-4" />
              </button>
              <div className="flex h-12 min-w-[280px] flex-1 items-center gap-2 rounded-md border border-[#d8dee4] bg-[#f6f8fa] px-4 text-sm text-[#6e7781]">
                <Search className="size-4" />
                <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search candidates, openings, notes, hires..." className="h-full flex-1 bg-transparent text-sm outline-none" />
              </div>
            </div>
                ...
                {stageRows.map(({ candidate, pipeline }) => (
                  ...
                  >
                    ...
                    <div className="mt-3 grid gap-2 text-xs text-[#57606a]">
                      <div><span className="font-black text-[#24292f]">Salary:</span> {salaryText(candidate)}</div>
    ...
    <div className="overflow-x-auto rounded-lg border border-[#d8dee4] bg-white">
      <table className="w-full min-w-[980px] text-left text-sm">
        <thead className="bg-[#f6f8fa] text-xs font-black uppercase tracking-wider text-[#57606a]">
          <tr><th className="px-4 py-3">Hire</th><th className="px-4 py-3">Role</th><th className="px-4 py-3">Service</th><th className="px-4 py-3">Start</th><th className="px-4 py-3">Salary</th><th className="px-4 py-3">Sales price</th><th className="px-4 py-3">Status</th></tr>
        </thead>
...
function CandidateDetail({
  ...
  return (
    <div>
      ...
      <div className="mt-5 rounded-md border border-[#d8dee4] bg-[#f6f8fa] p-4">
        <p className="text-xs font-black uppercase tracking-[0.14em] text-[#12866E]">Recruiter contact</p>
        <p className="mt-2 font-black">{pipeline.recruiter || "Nearwork recruiting team"}</p>
        <p className="mt-1 text-sm text-[#57606a]">Use the notes panel for candidate-specific questions, or contact your account manager for urgent support.</p>
      </div>
