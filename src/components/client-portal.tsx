        ...
        return (
          <div key={item.label}>
            ...
            <div className="relative h-3 rounded-full bg-[#edf1f5]">
              ...
              <span className="block h-3 rounded-full bg-gradient-to-r from-[#12866E] to-[#8EDFD1]" style={{ width: `${candidate}%` }} />
            </div>
          </div>
        );
      })}
...
function ClientAiInsight({ candidate, compact = false }: { candidate: PortalCandidate; compact?: boolean }) {
  ...
  const client = insight?.client;
  if (!client) return null;
  return (
    <div className="mt-4 grid gap-4">
      <div className="rounded-lg border border-teal-100 bg-[#eef8f5] p-4">
          ...
          {candidate.aiReview?.client ? (
            ...
            <>
              ...
              <div className="mt-4 rounded-lg border border-[#d8dee4] bg-[#f6f8fa] p-4 text-sm text-[#57606a]">
                Assessment insights will appear here after Nearwork generates the review in staff.nearwork.co.
              </div>
            </>
          )}
        </Panel>
